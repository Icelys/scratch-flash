...
public class MotionAndPenPrims {
	...
	private function primPointTowardsPos(b:Array):void {
		var s:ScratchSprite = interp.targetSprite();
		var px:int = b[0];
		var py:int = b[1];
	...
	private function primGoTo(b:Array):void {
		var s:ScratchSprite = interp.targetSprite();
		if (s != null) moveSpriteTo(s, interp.numarg(b[0]), interp.numarg(b[2]));
	}
	...
	private function primSetPenSize(b:Array):void {
		...
		// Should we do something about the glitch with pen sizes set to 1?
	}
