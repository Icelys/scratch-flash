...
public class Interpreter {
	...
	private var specialTable:Dictionary;		// maps opcodes to special op functions
...
//		checkPrims();
		...
		if (wasRunning) {
			...
			if (b.isReporter) {
				...
				b.opFunction = function(b:Block):void {
					...
					app.showBubble(String(interp.arg(b, 0)), 0, p.x, p.y, reporter.getRect(app.stage).width);
				};
				b.args[0] = reporter;
			}
		...
		while (true) {
			...
			if (activeThread.block == null) {
 				...
 				if (activeThread.values.length) {
 					app.browserTrace(activeThread.values[0]);
 				}
 				return;
 			}
 			if (yield) {
	...
	private function initPrims():void {
		...
		specialTable["wait:elapsed:from:"]	= primWait;
 		specialTable["doForever"]		= function(b:*):* {
		...
		specialTable["getVar"]             = primVarName;
 ...
 +			var listArg:* = b[1];
	...
	private function primCall(b:Array):void {
		...
		activeThread.pushStateForBlock(b.isReporter ? report0Block : returnBlock);
 		activeThread.args = b;
 		startCmdList(proc);
	}
